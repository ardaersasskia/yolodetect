...
video_device='nvarguscamerasrc num-buffers=1 maxperf=true sensor-id=0 ! video/x-raw(memory:NVMM), width=1280, height=720, format=NV12, framerate=(fraction)60/1 ! nvvidconv ! video/x-raw, format=(string)BGRx ! videoconvert ! appsink max-buffers=1 drop=true'
